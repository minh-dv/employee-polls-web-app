...
export function _getUser(username, password) {
...
export async function getUser(uname, pword) {
  try {
    const user = await _getUser(uname, pword);
    return user;
  } catch (error) {
    console.error(error);
  }
...
export async function saveQuestion(optionOneText, optionTwoText, author) {
  try {
...
export async function saveQuestionAnswer(authedUser, qid, answer) {
  try {
    const questionAnswer = await _saveQuestionAnswer({
      authedUser,
      qid,
      answer,
    });
